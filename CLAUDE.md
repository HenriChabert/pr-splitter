...
uv run pytest                  # Run all tests (49 tests)
...
src/pr_splitter/
  ...
  github.py    — GitHub integration via `gh` subprocess (push_branch, create_pr, create_all_prs)
...
- **Round-robin distribution** on alphabetically sorted files — deterministic, keeps same-directory files together.
...
Tests use temporary git repos created via pytest fixtures (see `tests/conftest.py`). GitHub tests mock subprocess calls. CLI tests use Click's `CliRunner`.
